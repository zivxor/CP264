...
void test_selection_sort() {
	...
	for (int i = 0; i < count; i++) {
		...
		printf("): ");
		selection_sort((void*) a, left, right);
		display_array(a, left, right);
		printf("\n");
	}
	printf("\n");
...
void test_cmp_sort() {
	...
	for (int i = 0; i < count; i++) {
		...
		display_array(a, left, right);
		printf(" cmp_inc): ");
		cmp_sort((void*) a, left, right, cmp_inc);
		display_array(a, left, right);
		printf("\n");
	}
	...
	for (int i = 0; i < count; i++) {
		...
		display_array(a, left, right);
		printf(" cmp_dec): ");
		cmp_sort((void*) a, left, right, cmp_dec);
		display_array(a, left, right);
		printf("\n");
	}
...
//run time measuring for selection_sort
	...
	for (int i = 0; i < m1; i++) {
		...
		selection_sort((void*) a, 0, MAX_LEN - 1);
	}
	clock_t t2 = clock();
...
//run time measuring for quick_sort
	...
	printf(
			"time_span(selection_sort(%d numbers))/time_span(quick_sort(%d numbers)):%0.1f\n",
